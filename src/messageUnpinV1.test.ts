...
beforeEach(() => {
  ...
  messagePinReq(a1.token, m1);
  messagePinReq(a1.token, dm1);
});
...
function messagePinReq(token: string, messageId: number) {
  try {
    const res = request(
      'POST',
              `${config.url}:${config.port}/message/pin/v1`,
              {
                json: {
                  messageId: messageId,
                },
...
function messageUnpinReq(token: string, messageId: number) {
  try {
    const res = request(
      'POST',
                `${config.url}:${config.port}/message/unpin/v1`,
                {
                  json: {
                    messageId: messageId,
                  },
...
describe('///// TESTING MESSAGEUNPIN /////', () => {
  test('Success -> AuthUser Unpin Channel Message', () => {
    const a = messageUnpinReq(a1.token, m1);
    expect(a.statusCode).toBe(OK);
    expect(a.body).toEqual({});
  });
  test('Success -> AuthUser Unpin DM Message', () => {
    const a = messageUnpinReq(a1.token, dm1);
    expect(a.statusCode).toBe(OK);
    expect(a.body).toEqual({});
  });
  test('Fail -> Invalid Token', () => {
    const a = messageUnpinReq('', m1);
    expect(a.statusCode).toBe(403);
  });
  test('Fail -> Invalid messageId', () => {
    const a = messageUnpinReq(a1.token, -1);
  ...
  test('Fail -> AuthUser leaves DM where Message exists', () => {
    ...
    const a = messageUnpinReq(a2.token, dm1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> Message is removed before Unpin', () => {
    messageRemoveReq(a1.token, m1);
    const a = messageUnpinReq(a1.token, m1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> DM is removed before Unpin', () => {
    DMRemoveReq(a1.token, d1);
    const a = messageUnpinReq(a1.token, dm1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> AuthUser has not joined Channel MessageId exists in', () => {
    const a = messageUnpinReq(a3.token, m1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> AuthUser has not joined DM MessageId exists in', () => {
    const a = messageUnpinReq(a4.token, dm1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> AuthUser does not have Owner Permissions in Channel', () => {
    postChannelJoin(a2.token, c1);
    const a = messageUnpinReq(a2.token, m1);
    expect(a.statusCode).toBe(403);
  });
  test('Fail -> AuthUser does not have Owner Permissions in DM', () => {
    const a = messageUnpinReq(a2.token, dm1);
    expect(a.statusCode).toBe(403);
  });
  test('Fail -> AuthUser is removed as Owner in DM', () => {
    DMLeaveReq(a1.token, d1);
    const a = messageUnpinReq(a1.token, dm1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> Message is already Unpinned', () => {
    messageUnpinReq(a1.token, m1);
    const a = messageUnpinReq(a1.token, m1);
    expect(a.statusCode).toBe(400);
  });
});
