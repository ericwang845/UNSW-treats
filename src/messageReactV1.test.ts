...
describe('///// TESTING MESSAGEREACT /////', () => {
  test('Success -> AuthUser reacts to Channel Message', () => {
    const a = messageReactReq(a1.token, m1, 1);
    expect(a.statusCode).toBe(OK);
    expect(a.body).toEqual({});
  });
  test('Success -> AuthUser reacts to DM Message', () => {
    const a = messageReactReq(a1.token, dm1, 1);
    expect(a.statusCode).toBe(OK);
    expect(a.body).toEqual({});
  });
  test('Fail -> Invalid Token', () => {
    const a = messageReactReq('', m1, 1);
    expect(a.statusCode).toBe(403);
  });
  test('Fail -> Invalid messageId', () => {
    const a = messageReactReq(a1.token, -1, 1);
  ...
  test('Fail -> AuthUser leaves DM where Message exists', () => {
    ...
    const a = messageReactReq(a2.token, dm1, 1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> Message is removed before React', () => {
    messageRemoveReq(a1.token, m1);
    const a = messageReactReq(a1.token, m1, 1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> DM is removed before React', () => {
    DMRemoveReq(a1.token, d1);
    const a = messageReactReq(a1.token, dm1, 1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> AuthUser has not joined Channel MessageId exists in', () => {
    const a = messageReactReq(a3.token, m1, 1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> AuthUser has not joined DM MessageId exists in', () => {
    const a = messageReactReq(a4.token, dm1, 1);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> Invalid ReactId ', () => {
    const a = messageReactReq(a1.token, dm1, 2);
    expect(a.statusCode).toBe(400);
  });
  test('Fail -> AuthUser has already Reacted', () => {
