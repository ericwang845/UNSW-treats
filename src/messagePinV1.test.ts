...
describe('///// TESTING MESSAGEPIN /////', () => {
  test('Success -> AuthUser Pins Channel Message', () => {
    const a = messagePinReq(a1.token, m1);
    expect(a.statusCode).toBe(OK);
    expect(a.body).toEqual({});
  });
  test('Success -> AuthUser Pins DM Message', () => {
    const a = messagePinReq(a1.token, dm1);
    expect(a.statusCode).toBe(OK);
    expect(a.body).toEqual({});
  });
  test('Fail -> Invalid Token', () => {
    const a = messagePinReq('', m1);
    expect(a.statusCode).toBe(403);
  });
  test('Fail -> Invalid messageId', () => {
    const a = messagePinReq(a1.token, -1);
  ...
  test('Fail -> Message is already Pinned', () => {
